...
for (const arkFile of scene.getFiles()) {
    if (!arkFile.getName().endsWith('try_finally_1032.ts')) continue;
    ...
    for (const arkClass of arkFile.getClasses()) {
        for (const arkMethod of arkClass.getMethods()) {
            ...
            for (let i = 0; i < blocks.length; i++) {
                console.log(`  label${i}:`);
                for (const stmt of blocks[i].getStmts()) {
                    const pos = stmt.getOriginFullPosition();
                    console.log(`      [line ${pos?.getFirstLine()}]  ${stmt.toString()}`);
                }
            }
            console.log();
        }
