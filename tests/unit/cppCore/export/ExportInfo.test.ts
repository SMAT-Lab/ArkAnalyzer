...
describe('export Test', () => {
    it('function implement in header file case1', () => {
        ...
        assert.equal(file1?.getImportInfos().length, 4);
    ...
    it('function implement in cpp file case', () => {
        ...
        if (stmts && is_system_win32) {
            ...
            assert.equal(stmts[10].getDef()?.getType().toString(), '@funcImplementInCpp/include/test.h: Circle');
            assert.equal(stmts[11].getInvokeExpr()?.getMethodSignature().toString(), '@funcImplementInCpp/include/test.h: Circle.CalculateArea()');
            assert.equal(stmts[15].getInvokeExpr()?.getMethodSignature().toString(), '@funcImplementInCpp/include/test.h: Circle.PrintInfo()');
        }
        ...
        assert.equal(file3?.getImportInfos().length, 6);
