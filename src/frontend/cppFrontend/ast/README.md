...
仓库根目录执行 `npm test` 时，`tests/unit/core_cpp` 下的用例依赖可执行的 **astJsonDumper**（查找路径见 `src/frontend/cpp_frontend/ast/const.ts` 中的 `getAstJsonDumperPath()` / `isAstJsonDumperAvailable()`）。构建完成后请将可执行文件放到 `src/frontend/cpp_frontend/ast/dumper/` 或 `lib/ast/`（与 `prepack` 拷贝规则一致）。若上述路径不存在该二进制，Vitest 将跳过 `tests/unit/core_cpp/**`，并在控制台输出提示。
