...
export class CfgBuilder {
    ...
    walkAST(lastStatement: StatementBuilder, nextStatement: StatementBuilder, nodes: CppAstNode[]): void {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (innerNode.kind === 'ReturnStmt') {
                break;
            } else if (innerNode.kind === 'BreakStmt' || innerNode.kind === 'ContinueStmt') {
                return;
            } else if (innerNode.kind === 'GotoStmt' || innerNode.kind === 'IndirectGotoStmt') {
