                        ...
                        bool isInclude, CXFile file)
{
    ...
    // ===== 7) code (on demand) =====
    if (w.code) {
        node["code"] = (hasContent && content.contains("code") && content["code"].is_string()) ? content["code"].get<std::string>() : "";
    }
    ...
    const ExtrasRefs xrefs{ hasContent ? &content : nullptr, file, NeedDisplayName(w) ? &displayName : nullptr, &fileStr };
