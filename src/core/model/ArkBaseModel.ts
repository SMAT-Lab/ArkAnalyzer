...
/**
 * Shift amount for class-specific tag encoding.
 * BaseModelTag uses bits 0-15 for shared properties.
 * Class-specific properties start at bit 16.
 */
export const CLASS_SPECIFIC_TAG_SHIFT = 16;

/**
 * BaseModelTag is a bitmask enumeration defining shared property tags.
 * These tags use bits 0-15 and are shared across multiple ArkBaseModel subclasses.
 *
 * Bit allocation:
 * - Bits 0-15: Shared property area (defined in BaseModelTag)
 * - Bits 16+: Class-specific property area (defined in each subclass file)
 *
 * Design principle:
 * - Shared semantic tags are defined here to avoid conflicts
 * - Class-specific tags are defined in respective subclass files using bit 16+
 */
export enum BaseModelTag {
    /**
     * Bit 0: Indicates whether a method is generated by the analyzer.
     * Used in ArkMethod to mark compiler-generated methods like default constructors.
     */
    GENERATED = 1,

    /**
     * Bit 1: Indicates whether a method has an asterisk token (generator function).
     * Used in ArkMethod for async generator methods.
     */
    ASTERISK_TOKEN = 1 << 1,

    /**
     * Bit 2: Indicates whether a method or field has a question token (optional modifier).
     * Shared semantic in ArkMethod and ArkField to mark optional members.
     */
    QUESTION_TOKEN = 1 << 2,

    /**
     * Bit 3: Indicates whether a field has an exclamation token (definite assignment assertion).
     * Used in ArkField to mark fields with '!' modifier.
     */
    EXCLAMATION_TOKEN = 1 << 3,
    // Bits 4-15: Reserved for future shared properties
}
...
export abstract class ArkBaseModel {
    ...
    public addTag(tag: BaseModelTag | number): void {
        this.tags = this.getTags() | tag;
    }

    public removeTag(tag: BaseModelTag): void {
        if (!this.tags) {
            return;
        }
        this.tags &= ~tag;
    ...
    public containsTag(tag: BaseModelTag | number): boolean {
        if (!this.tags) {
            return false;
        }
    ...
    protected setTagValue(mask: number, shift: number, value: number): void {
        this.tags = (this.getTags() & ~mask) | (value << shift);
    }

    protected getTagValue(mask: number, shift: number): number {
        return (this.getTags() & mask) >>> shift;
    }
